...
fun AdminExerciseStatisticScreen(
    ...
    // Фильтруем статистику по exerciseId
    val exerciseStatistics = statistics.filter { it.exerciseId == exerciseId }

    // Берем последнюю статистику для отображения
    val lastStatistic = exerciseStatistics.maxByOrNull { it.completedAt }

    val dataList = exerciseStatistics.mapIndexed { index, statistic ->
        BarData(
            value = statistic.mistakes.toFloat(), // кол-во ошибок
            label = "${exerciseStatistics.size - index}" // порядковый номер в обратном порядке
        )
    }
    ...
    if (lastStatistic == null) {
        ...
        val statisticListInfo = listOf(
            "Время ${"%.2f".format(lastStatistic.timeSpent.toDouble() / 1000)} сек",
            "Ошибки ${lastStatistic.mistakes}",
            "Среднее время нажатия ${"%.2f".format(lastStatistic.avgTime.toDouble())} мс"
        )
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Text(
                text = if (lastStatistic.isSuccessful) {
                    "Успешно пройдено"
                } else {
                    "Не пройдено"
                },
